...
import { View, Text, RefreshControl, ActivityIndicator, FlatList, TouchableOpacity, StatusBar, ListRenderItem } from 'react-native';
...
export default function CategoryDetailScreen() {
  ...
  const handleWallpaperPress = (wallpaper: PixabayImage) => {
    navigation.push('Detail', { wallpaper: JSON.stringify(wallpaper) });
  };
  ...
  const renderFooter = () => loadingMore ? <View className="py-8"><ActivityIndicator /></View> : null;
  ...
  return (
    <View style={{ paddingTop }} className="flex-1 bg-light-background dark:bg-dark-background">
      ...
      <View className="flex-row items-center justify-between px-4 pb-4">
        <TouchableOpacity onPress={handleBack} className="p-1"><ArrowLeftIcon size={24} className="text-light-text dark:text-dark-text" /></TouchableOpacity>
        <Text className="font-heading text-xl capitalize text-light-text dark:text-dark-text">{category}</Text>
        <View className="w-6" />
      </View>
